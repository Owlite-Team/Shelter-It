...
func (h *AuthHandler) Register(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Transaction start failed", "details": err.Error()})
		return
	}

	var userId int
	err = tx.QueryRow(`INSERT INTO "Users" (email, password_hash) VALUES ($1, $2) RETURNING user_id`, user.Email, hashedPassword).Scan(&userId)
	if err != nil {
		tx.Rollback()
...
func (h *AuthHandler) Login(c *gin.Context) {
	...
	var user dto.User
	err := h.db.DB.QueryRow(`SELECT user_id, email, password_hash FROM "Users" WHERE email = $1`, login.Email).Scan(&user.ID, &user.Email, &user.PasswordHash)
